        ...
        <?php
            ...
            if (!isset($pk) || trim($pk) == '') {
                ...
                //all input is valid
                    ...
                    $search_query = "SELECT c.Address, c.Item_PLU, i.Name, c.Form_Of_Payement, c.Purchase_Key, e.name, c.Name as cName FROM Customer AS c, ITEM as i, Employee as e WHERE c.Purchase_Key=\"".$pk."\" AND e.ssn=c.Delivery_Employee_SSN AND i.Item_PLU=c.Item_PLU;";
                    $search_result = $mysqli_conn->query($search_query);
                    if ($search_result === false || $search_result->num_rows == 0) {
                        echo "<h1 class='bodyText'>No order with corresponding purchase key exists. Please go back and try again.</h1><br/><p class='top_links'><a href='customerLookup.html' class='topLink'>Go Back</a></p>";
                    }
                    else {
                        while($customer_row = $search_result->fetch_assoc()) {
                            ...
                            echo "<p>Recipient: ".$customer_row["cName"]."</p>";
                            echo "<p>Payement Method: ".$customer_row["Form_Of_Payement"]."</p>";
                            echo "<p>Delivered By: ".$customer_row["name"]."</p>";
                            echo "<p class='top_links'><a href='orderLookup.html' class='topLink'>Look-Up Another Order</a></p>";
                            break;
                        }
                    }
                }
